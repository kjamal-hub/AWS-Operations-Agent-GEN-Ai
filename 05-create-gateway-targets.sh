...
if command -v yq >/dev/null 2>&1; then
    ...
    # Debian-compatible fallback: extract from YAML using grep/sed
    REGION=$(grep "region:" "${CONFIG_DIR}/static-config.yaml" | head -1 | sed 's/.*region:[[:space:]]*["\x27]*\([^"\x27]*\)["\x27]*$/\1/')
    ACCOUNT_ID=$(grep "account_id:" "${CONFIG_DIR}/static-config.yaml" | head -1 | sed 's/.*account_id:[[:space:]]*["\x27]*\([^"\x27]*\)["\x27]*$/\1/')
    ROLE_ARN=$(grep "role_arn:" "${CONFIG_DIR}/static-config.yaml" | head -1 | sed 's/.*role_arn:[[:space:]]*["\x27]*\([^"\x27]*\)["\x27]*$/\1/')
    # Get gateway execution role from dynamic config
    GATEWAY_EXECUTION_ROLE_ARN=$(grep "gateway_execution_role_arn:" "${CONFIG_DIR}/dynamic-config.yaml" | head -1 | sed 's/.*gateway_execution_role_arn:[[:space:]]*["\x27]*\([^"\x27]*\)["\x27]*$/\1/')
fi
...
if command -v yq >/dev/null 2>&1; then
    ...
    PROVIDER_ARN=$(grep "provider_arn:" "${CONFIG_DIR}/dynamic-config.yaml" | head -1 | sed 's/.*provider_arn:[[:space:]]*["\x27]*\([^"\x27]*\)["\x27]*$/\1/')
    OKTA_DOMAIN=$(grep "domain:" "${CONFIG_DIR}/static-config.yaml" | head -1 | sed 's/.*domain:[[:space:]]*["\x27]*\([^"\x27]*\)["\x27]*$/\1/')
fi
...
if command -v yq >/dev/null 2>&1; then
    ...
    JWT_DISCOVERY_URL=$(grep "discovery_url:" "${CONFIG_DIR}/static-config.yaml" | head -1 | sed 's/.*discovery_url:[[:space:]]*["\x27]*\([^"\x27]*\)["\x27]*$/\1/')
    JWT_AUDIENCE=$(grep "audience:" "${CONFIG_DIR}/static-config.yaml" | head -1 | sed 's/.*audience:[[:space:]]*["\x27]*\([^"\x27]*\)["\x27]*$/\1/')
fi
...
if command -v yq >/dev/null 2>&1; then
    ...
    LAMBDA_FUNCTION_ARN=$(grep "function_arn:" "${CONFIG_DIR}/dynamic-config.yaml" | head -1 | sed 's/.*function_arn:[[:space:]]*["\x27]*\([^"\x27]*\)["\x27]*$/\1/' 2>/dev/null)
fi
...
# Use configured AWS profile if specified in static config - Debian-compatible version
AWS_PROFILE_CONFIG=$(grep "aws_profile:" "${CONFIG_DIR}/static-config.yaml" | head -1 | sed 's/.*aws_profile:[[:space:]]*["\x27]*\([^"\x27#]*\)["\x27]*.*$/\1/' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' 2>/dev/null)
...
# Using more portable sed patterns for Debian
GATEWAY_ID=$(echo "$GATEWAY_RESPONSE" | grep "Gateway ID:" | sed 's/.*Gateway ID:[[:space:]]*//' | tail -1 | tr -d '\n\r')
GATEWAY_URL=$(echo "$GATEWAY_RESPONSE" | grep "Gateway URL:" | sed 's/.*Gateway URL:[[:space:]]*//' | tail -1 | tr -d '\n\r')
...
if command -v yq >/dev/null 2>&1; then
    ...
    GATEWAY_ARN=$(grep "arn:" "${CONFIG_DIR}/dynamic-config.yaml" | head -1 | sed 's/.*arn:[[:space:]]*["\x27]*\([^"\x27]*\)["\x27]*$/\1/' 2>/dev/null)
fi
...
# Extract Target information from response - Debian-compatible sed patterns
TARGET_ID=$(echo "$TARGET_RESPONSE" | grep "Target ID:" | sed 's/.*Target ID:[[:space:]]*//' | tail -1 | tr -d '\n\r')
TOOL_COUNT=$(echo "$TARGET_RESPONSE" | grep "Tool Count:" | sed 's/.*Tool Count:[[:space:]]*//' | tail -1 | tr -d '\n\r')
